...
SERIES = {
    ...
    "EUCBBS":    1000.0,   # EUR millions -> EUR billions
    "JPCBBS":    1000.0,   # JPY millions -> JPY billions
    "CNCBBS":    1000.0,   # CNY millions -> CNY billions
}
...
#   DEXCHUS = CNY per USD  -> DIVIDE   CNY to get USD
FX = {
    "EURUSD": "DEXUSEU",
    "JPYUSD": "DEXJPUS",
    "CNYUSD": "DEXCHUS",
...
FIELDS = ["date",
          ...
          "ecb_bs_bn_eur", "boj_bs_bn_jpy", "pboc_bs_bn_cny",
          # balance sheets converted to USD billions
          "ecb_bs_bn_usd", "boj_bs_bn_usd", "pboc_bs_bn_usd",
          # composites
          "global_bs_bn_usd",
...
def main():
    ...
    cnyusd = latest_on_or_before(fx["CNYUSD"], date)   # CNY per USD
    ...
    pboc_usd = pboc_cny / cnyusd if (pboc_cny is not None and cnyusd) else None

    # Composite 1: global CB balance sheet = Fed + ECB + BoJ + PBoC (USD bn)
    global_bs = (walcl + ecb_usd + boj_usd + pboc_usd
                 if None not in (walcl, ecb_usd, boj_usd, pboc_usd) else None)

    # Composite 2: US net liquidity + the three foreign CB balance sheets (USD bn)
    netliq_plus_cb = (net_liq + ecb_usd + boj_usd + pboc_usd
                      if None not in (net_liq, ecb_usd, boj_usd, pboc_usd) else None)

    # Composite 3: US net liquidity with the repo leg added back
    ...
    row = [date,
           ...
           fmt(net_liq),
           fmt(ecb_eur), fmt(boj_jpy), fmt(pboc_cny),
           fmt(ecb_usd), fmt(boj_usd), fmt(pboc_usd),
           fmt(global_bs),
           fmt(netliq_plus_cb),
           fmt(netliq_plus_rpo)]
